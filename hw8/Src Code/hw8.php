<?php 
    /****************************
	Simple php-DB Program for Review form Submisiion
	Ashwini Kulkarni
   ...
   // array of source
    ...
    if (isset($_POST["SignUP"]))
    {
	    ...
	    {
	    	 $op='Table Insertion';
    		 $sql = sprintf("INSERT INTO DIM_REVIEW (Name, Email, Referrer, Rating, Comments) VALUES ('%s','%s','%s','%s','%s')",
    		 $connection->real_escape_string($_POST["name"]),
             $connection->real_escape_string($_POST["email"]),
             $connection->real_escape_string($_POST["visitInterest"]),
             $connection->real_escape_string($_POST["source"]),
             $connection->real_escape_string($_POST["comment"])
    		 );
...
<html>
    ...
    <form action="hw8.php" method="post">    
	    ...
	    <div><input type="Submit" value="Submit Review" name="SignUP"></div>
